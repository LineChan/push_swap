...
sub process_file
{
	...
	while( my $line = <FILE>)
	{
		...
		{	
			...
			{
				print "[\e[31mFAILURE\e[0m] bad pushswap\n";
				if ($verbose)
				{
					print STDERR "$arg\n$ops\n";
			...
			{
				printf("[\e[32mSUCCESS\e[0m] %d operations\n", $ops =~ tr/ // + 1);
				perror_list("List A", \@list_a);
				perror_list("List B", \@list_b);
			}
		}
...
sub process_list
{
	...
	if (sorted(@list_a) == 0 || @list_b)
	{
		print "[\e[31mFAILURE\e[0m] bad pushswap on $_[0] items\n";
		if ($verbose)
		{
			print STDERR "$arg\n$ops\n";
			perror_list("List A", \@list_a);
	...
	{
		printf("[\e[32mSUCCESS\e[0m] %d operations ", $ops =~ tr/ // + 1);
